...
using DeliveryTracker.MOEA.Utils.Parallel;
...
namespace DeliveryTracker.MOEA.Metaheuristics.NSGAII
{
	...
	public class PNSGAII : Algorithm
	{
		...
		public PNSGAII(Problem problem, ISynchronousParallelRunner evaluator)
			: base(problem)
		{
			this.parallelEvaluator = evaluator;
		}
		...
		public override SolutionSet Execute()
		{
			...
			this.parallelEvaluator.StartParallelRunner(this.Problem); ;
			...
			for (int i = 0; i < populationSize; i++)
			{
				...
				this.parallelEvaluator.AddTaskForExecution(new object[] { newSolution }); ;
			}

			List<Solution> solutionList = (List<Solution>)this.parallelEvaluator.ParallelExecution();
			...
			while (evaluations < maxEvaluations)
			{
				...
				for (int i = 0; i < (populationSize / 2); i++)
				{
					if (evaluations < maxEvaluations)
					{
						...
						this.parallelEvaluator.AddTaskForExecution(new object[] { offSpring[0] }); ;
						this.parallelEvaluator.AddTaskForExecution(new object[] { offSpring[1] }); ;
					}
				}

				List<Solution> solutions = (List<Solution>)this.parallelEvaluator.ParallelExecution();
