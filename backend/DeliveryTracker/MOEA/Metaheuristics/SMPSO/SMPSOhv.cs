...
using DeliveryTracker.MOEA.QualityIndicator.FastHypervolume;
...
namespace DeliveryTracker.MOEA.Metaheuristics.SMPSO
{
	...
	public class SMPSOhv : Algorithm
	{
			...
			: base(problem)
		{
			...
			this.trueHypervolume = this.hy.Hypervolume(this.trueFront.WriteObjectivesToMatrix(), this.trueFront.WriteObjectivesToMatrix(), problem.NumberOfObjectives);
		...
		public void InitParams()
		{
			...
			this.Leaders = new FastHypervolumeArchive(this.archiveSize, this.Problem.NumberOfObjectives);
		...
		public override SolutionSet Execute()
		{
			...
			this.Leaders.ComputeHVContribution();
			...
			while (this.iteration < this.maxIterations)
			{
				...
				if (this.Leaders.Size() < this.archiveSize)
					this.Leaders.ComputeHVContribution();
		...
		private void ComputeSpeed(int iter, int miter)
		{
			...
			for (int i = 0; i < this.swarmSize; i++)
			{
				...
				for (int var = 0; var < particle.GetNumberOfDecisionVariables(); var++)
				{
					...
					this.speed[i][var] = this.VelocityConstriction(this.ConstrictionCoefficient(C1, C2) * (this.InertiaWeight(iter, miter, wmax, wmin) * this.speed[i][var] + C1 * r1 * (bestParticle.GetValue(var) - particle.GetValue(var)) + C2 * r2 * (bestGlobal.GetValue(var) - particle.GetValue(var))),
						this.deltaMax,
									this.deltaMin,
									var,
									i);
