...
namespace DeliveryTracker.MOEA.Metaheuristics.SMSEMOA
{
	/// <summary>
	...
	public class SMSEMOA : Algorithm
	{
		...
		public override SolutionSet Execute()
		{
			...
			Utils.Utils.GetDoubleValueFromParameter(this.InputParameters, "offset", ref offset);
			...
			while (evaluations < maxEvaluations)
			{
				...
				if (lastFront.Size() > 1)
				{
					...
					int numberOfObjectives = this.Problem.NumberOfObjectives;
					// STEP 1. Obtain the maximum and minimum values of the Pareto front
					double[] maximumValues = this.utils.GetMaximumValues(union.WriteObjectivesToMatrix(), numberOfObjectives);
					double[] minimumValues = this.utils.GetMinimumValues(union.WriteObjectivesToMatrix(), numberOfObjectives);
					// STEP 2. Get the normalized front
					double[][] normalizedFront = this.utils.GetNormalizedFront(frontValues, maximumValues, minimumValues);
					...
					double[][] invertedFront = this.utils.InvertedFront(normalizedFront);
		...
		private double[] HvContributions(double[][] front)
		{
			...
			double totalVolume = this.hv.CalculateHypervolume(totalFront, totalFront.Length, numberOfObjectives);
			for (int i = 0; i < front.Length; i++)
			{
				...
				double chv = this.hv.CalculateHypervolume(frontSubset, frontSubset.Length, numberOfObjectives);
