...
namespace DeliveryTracker.MOEA.Operators.Crossover
{
	...
	public class DifferentialEvolutionCrossover : Crossover
	{
			...
			: base(parameters)
		{
			...
			this.f = DEFAULT_F;
			this.k = DEFAULT_K;
			this.deVariant = DEFAULT_DE_VARIANT;


			Utils.Utils.GetDoubleValueFromParameter(parameters, "CR", ref this.cr);
			Utils.Utils.GetDoubleValueFromParameter(parameters, "F", ref this.f);
			Utils.Utils.GetDoubleValueFromParameter(parameters, "K", ref this.k);
			Utils.Utils.GetStringValueFromParameter(parameters, "DE_VARIANT", ref this.deVariant);
		}
		...
		public override object Execute(object obj)
		{
			...
			if ((this.deVariant == "rand/1/bin") || (this.deVariant == "best/1/bin"))
			{
				for (int j = 0; j < numberOfVariables; j++)
				{
					if (JMetalRandom.NextDouble(0, 1) < this.cr || j == jrand)
					{
						double value;
						value = xParent2.GetValue(j) + this.f * (xParent0.GetValue(j) - xParent1.GetValue(j));
			...
			else if ((this.deVariant == "rand/1/exp") || (this.deVariant == "best/1/exp"))
			{
				for (int j = 0; j < numberOfVariables; j++)
				{
					if (JMetalRandom.NextDouble(0, 1) < this.cr || j == jrand)
			...
			else if ((this.deVariant == "current-to-rand/1") || (this.deVariant == "current-to-best/1"))
			{
				for (int j = 0; j < numberOfVariables; j++)
				{
					double value;
					value = xCurrent.GetValue(j) + this.k * (xParent2.GetValue(j)
							- xCurrent.GetValue(j))
							+ this.f * (xParent0.GetValue(j) - xParent1.GetValue(j));
			...
			else if ((this.deVariant == "current-to-rand/1/bin") || (this.deVariant == "current-to-best/1/bin"))
			{
				for (int j = 0; j < numberOfVariables; j++)
				{
					if (JMetalRandom.NextDouble(0, 1) < this.cr || j == jrand)
			...
			else if ((this.deVariant == "current-to-rand/1/exp") || (this.deVariant == "current-to-best/1/exp"))
			{
				for (int j = 0; j < numberOfVariables; j++)
				{
					if (JMetalRandom.NextDouble(0, 1) < this.cr || j == jrand)
