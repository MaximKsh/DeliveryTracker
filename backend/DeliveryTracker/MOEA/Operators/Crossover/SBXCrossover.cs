...
namespace DeliveryTracker.MOEA.Operators.Crossover
{
	...
	public class SBXCrossover : Crossover
	{
		...
		public SBXCrossover(Dictionary<string, object> parameters)
			: base(parameters)
		{
			Utils.Utils.GetDoubleValueFromParameter(parameters, "probability", ref this.crossoverProbability);
				...
				for (i = 0; i < numberOfVariables; i++)
				{
					...
					if (JMetalRandom.NextDouble() <= 0.5)
					{
						if (Math.Abs(valueX1 - valueX2) > EPS)
						{
							...
							if (rand <= (1.0 / alpha))
							{
								betaq = Math.Pow((rand * alpha), (1.0 / (this.distributionIndex + 1.0)));
							}
							else
							{
								betaq = Math.Pow((1.0 / (2.0 - rand * alpha)), (1.0 / (this.distributionIndex + 1.0)));
							}
		...
		public override object Execute(object obj)
		{
			...
			Solution[] offSpring;
			offSpring = this.DoCrossover(this.crossoverProbability, parents[0], parents[1]);
