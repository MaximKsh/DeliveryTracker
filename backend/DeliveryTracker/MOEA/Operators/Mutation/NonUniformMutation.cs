...
namespace DeliveryTracker.MOEA.Operators.Mutation
{
	...
	public class NonUniformMutation : Mutation
	{
		...
		public NonUniformMutation(Dictionary<string, object> parameters)
			: base(parameters)
		{
			Utils.Utils.GetDoubleValueFromParameter(parameters, "probability", ref this.mutationProbability);
			Utils.Utils.GetDoubleValueFromParameter(parameters, "perturbation", ref this.perturbation);
			Utils.Utils.GetIntValueFromParameter(parameters, "maxIterations", ref this.maxIterations);
		}
		...
		private void DoMutation(double probability, Solution solution)
		{
			...
			for (int var = 0; var < solution.Variable.Length; var++)
			{
				if (JMetalRandom.NextDouble() < probability)
				{
					...
					if (rand <= 0.5)
					{
						tmp = this.Delta(x.GetUpperBound(var) - x.GetValue(var), this.perturbation.Value);
						tmp += x.GetValue(var);
					}
					else
					{
						tmp = this.Delta(x.GetLowerBound(var) - x.GetValue(var), this.perturbation.Value);
						tmp += x.GetValue(var);
					}
		...
		private double Delta(double y, double bMutationParameter)
		{
			double rand = JMetalRandom.NextDouble();
			int it, maxIt;
			it = this.currentIteration.Value;
		...
		public override object Execute(object obj)
		{
			...
			{
				this.currentIteration = (int)this.GetParameter("currentIteration");
			}

			this.DoMutation(this.mutationProbability.Value, solution);
