...
namespace DeliveryTracker.MOEA.Problems.LZ09
{
	...
	public class LZ09
	{
		...
		double Psfunc2(double x, double t1, int dim, int type, int css)
		{
			...
			{
				double xy = 2 * (x - 0.5);
				beta = xy - Math.Pow(t1, 0.5 * (this.nvar + 3 * dim - 8) / (this.nvar - 2));
			}

			if (type == 22)
			{
				double theta = 6 * Math.PI * t1 + dim * Math.PI / this.nvar;
				double xy = 2 * (x - 0.5);
		...
		double psfunc3(double x, double t1, double t2, int dim, int type)
		{
			...
			{
				double xy = 4 * (x - 0.5);
				double rate = 1.0 * dim / this.nvar;
				beta = xy - 4 * (t1 * t1 * rate + t2 * (1.0 - rate)) + 2;
			}

			if (type == 32)
			{
				double theta = 2 * Math.PI * t1 + dim * Math.PI / this.nvar;
				double xy = 4 * (x - 0.5);
		...
		public void Objective(List<double> x_var, List<double> y_obj)
		{
			...
			if (this.nobj == 2)
			{
				if (this.ltype == 21 || this.ltype == 22 || this.ltype == 23 || this.ltype == 24 || this.ltype == 26)
				{
					...
					for (int n = 1; n < this.nvar; n++)
					{
						...
						if (n % 2 == 0)
						{
							a = this.Psfunc2(x_var[n], x_var[0], n, this.ltype, 1);  // linkage
							aa.Add(a);
						}
						else
						{
							b = this.Psfunc2(x_var[n], x_var[0], n, this.ltype, 2);
							bb.Add(b);
						}
					...
					this.AlphaFunction(alpha, x_var, 2, this.ptype);  // shape function
					y_obj[0] = alpha[0] + h;
					y_obj[1] = alpha[1] + g;
					aa.Clear();
					bb.Clear();
				...
				if (this.ltype == 25)
				{
					...
					for (int n = 1; n < this.nvar; n++)
					{
						if (n % 3 == 0)
						{
							a = this.Psfunc2(x_var[n], x_var[0], n, this.ltype, 1);
							aa.Add(a);
						}
						else if (n % 3 == 1)
						{
						...
						{
							c = this.Psfunc2(x_var[n], x_var[0], n, this.ltype, 3);
							if (n % 2 == 0)
							{
								aa.Add(c);
							}
					...
					g = this.BetaFunction(aa, this.dtype);          // distance function
					h = this.BetaFunction(bb, this.dtype);
					double[] alpha = new double[2];
					this.AlphaFunction(alpha, x_var, 2, this.ptype);  // shape function
					y_obj[0] = alpha[0] + h;
			...
			if (this.nobj == 3)
			{
				if (this.ltype == 31 || this.ltype == 32)
				{
					...
					{
						a = this.psfunc3(x_var[n], x_var[0], x_var[1], n, this.ltype);
						if (n % 3 == 0)
						{
							aa.Add(a);
						}
					...
					this.AlphaFunction(alpha, x_var, 3, this.ptype);  // shape function
					y_obj[0] = alpha[0] + h;
					y_obj[1] = alpha[1] + g;
					y_obj[2] = alpha[2] + e;
					aa.Clear();
