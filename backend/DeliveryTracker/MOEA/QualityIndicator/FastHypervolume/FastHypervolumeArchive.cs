...
namespace DeliveryTracker.MOEA.QualityIndicator.FastHypervolume
{
	...
	public class FastHypervolumeArchive : Archive
	{
			...
			: base(maxSize)
		{
			...
			this.ReferencePoint = new Solution(this.objectives);
			for (int i = 0; i < this.objectives; i++)
			{
				this.ReferencePoint.Objective[i] = double.MaxValue;
			}

			this.crowdingDistance = new CrowdingComparator();
		}
		...
		public override bool Add(Solution solution)
		{
			...
			while (i < this.SolutionsList.Count)
			{
				...
				flag = this.dominance.Compare(solution, aux);
				if (flag == 1)
				...
				{
					if (this.@equals.Compare(aux, solution) == 0)
			...
			{ // The archive is full
				...
				this.Remove(this.IndexWorst(this.crowdingDistance));
			}
			return true;
		}
