...
namespace DeliveryTracker.MOEA.QualityIndicator.FastHypervolume.WFG
{
	public class WFGHV
	{
		...
		public WFGHV(int dimension, int maxNumberOfPoints)
		{
			...
			int maxd = maxNumberOfPoints - (this.OPT / 2 + 1);
		...
		public double GetHV(Front front, Solution refPoint)
		{
			...
			else
			{
				...
				{
					volume += Math.Abs(front.Points[i].Objectives[this.currentDimension] - this.referencePoint.Objectives[this.currentDimension]) * this.GetExclusiveHV(front, i);
				}
				this.currentDimension++;
			}
		...
		public double Get2DHV(Front front)
		{
			...
			hv = Math.Abs((front.Points[0].Objectives[0] - this.referencePoint.Objectives[0]) * (front.Points[0].Objectives[1] - this.referencePoint.Objectives[1]));

			for (int i = 1; i < front.NPoints; i++)
			{
				hv += Math.Abs((front.Points[i].Objectives[0] - this.referencePoint.Objectives[0]) * (front.Points[i].Objectives[1] - front.Points[i - 1].Objectives[1]));
		...
		public double GetInclusiveHV(Point p)
		{
			...
			{
				volume *= Math.Abs(p.Objectives[i] - this.referencePoint.Objectives[i]);
			}
		...
		public double GetExclusiveHV(Front front, int point)
		{
			...
			volume = this.GetInclusiveHV(front.Points[point]);
		...
		public void MakeDominatedBit(Front front, int p)
		{
			...
			for (int i = 0; i < z; i++)
				...
				{
					this.fs[this.currentDeep].Points[i].Objectives[j] = this.Worse(front.Points[p].Objectives[j], front.Points[p + 1 + i].Objectives[j], false);
				}

			Point t;
			this.fs[this.currentDeep].NPoints = 1;

			for (int i = 1; i < z; i++)
			{
				int j = 0;
				bool keep = true;
				while (j < this.fs[this.currentDeep].NPoints && keep)
				{
					switch (this.Dominates2way(this.fs[this.currentDeep].Points[i], this.fs[this.currentDeep].Points[j]))
					{
						case -1:
							t = this.fs[this.currentDeep].Points[j];
							this.fs[this.currentDeep].NPoints--;
							this.fs[this.currentDeep].Points[j] = this.fs[this.currentDeep].Points[this.fs[this.currentDeep].NPoints];
							this.fs[this.currentDeep].Points[this.fs[this.currentDeep].NPoints] = t;
							break;
						case 0:
							j++;
							break;
				...
				if (keep)
				{
					t = this.fs[this.currentDeep].Points[this.fs[this.currentDeep].NPoints];
					this.fs[this.currentDeep].Points[this.fs[this.currentDeep].NPoints] = this.fs[this.currentDeep].Points[i];
					this.fs[this.currentDeep].Points[i] = t;
					this.fs[this.currentDeep].NPoints++;
				}
			}
		...
		{
			Array.Sort(front.Points, 0, front.NPoints, this.pointComparator);
		}
