	...
	public class HyperVolume
	{
		...
		public double CalculateHypervolume(double[][] front, int noPoints, int noObjectives)
		{
			...
			while (n > 0)
			{
				...
				noNondominatedPoints = this.FilterNondominatedSet(front, n, noObjectives - 1);
				if (noObjectives < 3)
				{
				...
				{
					tempVolume = this.CalculateHypervolume(front, noNondominatedPoints,
							noObjectives - 1);
				}
				try
				{
		...
		public double Hypervolume(double[][] paretoFront, double[][] paretoTrueFront, int numberOfObjectives)
		{
			...
			maximumValues = this.utils.GetMaximumValues(paretoTrueFront, numberOfObjectives);
			minimumValues = this.utils.GetMinimumValues(paretoTrueFront, numberOfObjectives);

			// STEP 2. Get the normalized front
			normalizedFront = this.utils.GetNormalizedFront(paretoFront,
					maximumValues,
					minimumValues);

			// STEP 3. Inverse the pareto front. This is needed because of the original
			//metric by Zitzler is for maximization problems
			invertedFront = this.utils.InvertedFront(normalizedFront);
		...
		private int FilterNondominatedSet(double[][] front, int noPoints, int noObjectives)
		{
			...
			while (i < n)
			{
				...
				while (j < n)
				{
					...
					else if (this.Dominates(front[j], front[i], noObjectives))
					{
