...
namespace DeliveryTracker.MOEA.QualityIndicator
{
	...
	public class InvertedGenerationalDistance
	{
		...
		public double CalculateInvertedGenerationalDistance(double[][] front, double[][] trueParetoFront, int numberOfObjectives)
		{
			...
			normalizedFront = this.utils.GetNormalizedFront(front,
					maximumValue,
					minimumValue);
			normalizedParetoFront = this.utils.GetNormalizedFront(trueParetoFront,
					maximumValue,
					minimumValue);
			...
			{
				sum += Math.Pow(this.utils.DistanceToClosedPoint(aNormalizedParetoFront, normalizedFront), pow);
			}
