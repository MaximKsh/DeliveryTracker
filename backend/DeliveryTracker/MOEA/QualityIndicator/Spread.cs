...
namespace DeliveryTracker.MOEA.QualityIndicator
{
	...
	public class Spread
	{
		...
		public double CalculateSpread(double[][] front, double[][] trueParetoFront, int numberOfObjectives)
		{
			...
			// the metric)
			double df = this.utils.Distance(normalizedFront[0], normalizedParetoFront[0]);
			double dl = this.utils.Distance(normalizedFront[normalizedFront.Length - 1],
			...
			{
				mean += this.utils.Distance(normalizedFront[i], normalizedFront[i + 1]);
			}
			...
			if (numberOfPoints > 1)
			{
				...
				{
					diversitySum += Math.Abs(this.utils.Distance(normalizedFront[i], normalizedFront[i + 1]) - mean);
				}
