...
namespace DeliveryTracker.MOEA.QualityIndicator.Utils
{
	/// <summary>
	...
	public class MetricsUtil
	{
		...
		public double[] HVContributions(SolutionSet[] populations)
		{
			...
			for (int i = 0; i < normalizedFront.Length; i++)
			{
				...
				{
					normalizedFront[i] = this.GetNormalizedFront(frontValues[i], maximumValues, minimumValues);
				}
				else
				{
			...
			for (int i = 0; i < invertedFront.Length; i++)
			{
				...
				{
					invertedFront[i] = this.InvertedFront(normalizedFront[i]);
				}
				else
				{
