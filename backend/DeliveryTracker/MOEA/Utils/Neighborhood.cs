	...
	public class Neighborhood
	{
		...
		public Neighborhood(int solutionSetSize)
		{
			...
			for (int ind = 0; ind < solutionSetSize; ind++)
			{
				for (int radio = 0; radio < MAXRADIO; radio++)
				{
					...
					{//neighboors whit rate 1
						this.structure[ind][radio] = new int[8];
					}
					else if (radio == 1)
					{ //neighboors whit rate 2
						this.structure[ind][radio] = new int[24];
					}
				}
			}

			//Calculate the size of a row
			this.rowSize = (int)Math.Sqrt((double)solutionSetSize);
			...
			for (int ind = 0; ind < solutionSetSize; ind++)
			{
				//rate 1
				//North neighbors
				if (ind > this.rowSize - 1)
				{
					this.structure[ind][0][(int)Row.N] = ind - this.rowSize;
				}
				else
				{
					this.structure[ind][0][(int)Row.N] = (ind - this.rowSize + solutionSetSize) % solutionSetSize;
				}

				//East neighbors
				if ((ind + 1) % this.rowSize == 0)
					this.structure[ind][0][(int)Row.E] = (ind - (this.rowSize - 1));
				else
					this.structure[ind][0][(int)Row.E] = (ind + 1);

				//Western neigbors
				if (ind % this.rowSize == 0)
				{
					this.structure[ind][0][(int)Row.W] = (ind + (this.rowSize - 1));
				}
				else
				{
					this.structure[ind][0][(int)Row.W] = (ind - 1);
				}

				//South neigbors
				this.structure[ind][0][(int)Row.S] = (ind + this.rowSize) % solutionSetSize;
			}

			for (int ind = 0; ind < solutionSetSize; ind++)
			{
				this.structure[ind][0][(int)Row.NE] = this.structure[this.structure[ind][0][(int)Row.N]][0][(int)Row.E];
				this.structure[ind][0][(int)Row.NW] = this.structure[this.structure[ind][0][(int)Row.N]][0][(int)Row.W];
				this.structure[ind][0][(int)Row.SE] = this.structure[this.structure[ind][0][(int)Row.S]][0][(int)Row.E];
				this.structure[ind][0][(int)Row.SW] = this.structure[this.structure[ind][0][(int)Row.S]][0][(int)Row.W];
			}
		}
		...
		public SolutionSet GetFourNeighbors(SolutionSet solutionSet, int location)
		{
			...
			//North
			index = this.structure[location][0][(int)Row.N];
			neighbors.Add(solutionSet.Get(index));

			//South
			index = this.structure[location][0][(int)Row.S];
			neighbors.Add(solutionSet.Get(index));

			//East
			index = this.structure[location][0][(int)Row.E];
			neighbors.Add(solutionSet.Get(index));

			//West
			index = this.structure[location][0][(int)Row.W];
			neighbors.Add(solutionSet.Get(index));
