	...
	public class DifferentialEvolutionOffspring : Offspring
	{
		...
		public DifferentialEvolutionOffspring(double CR, double F)
		{
			...
			try
			{
				...
				parameters.Add("F", F);
				this.crossover = new DifferentialEvolutionCrossover(parameters);

				// Selecion operator
				parameters = null;
				this.selection = SelectionFactory.GetSelectionOperator("DifferentialEvolutionSelection", parameters);
			}
			catch (Exception ex)
			{
		...
		public override Solution GetOffspring(SolutionSet solutionSet, int index)
		{
			...
			try
			{
				...
				offSpring = (Solution)this.crossover.Execute(new object[] { solutionSet.Get(index), parents });
			}
			catch (Exception ex)
			{
		...
		public override Solution GetOffspring(Solution[] parentSolutions, Solution currentSolution)
		{
			...
			try
			{
				...
				offspring = (Solution)this.crossover.Execute(new object[] { currentSolution, parents });
			}
			catch (Exception ex)
			{
				Logger.Log.Error("Exception in " + this.GetType().FullName + ".GetOffSpring()", ex);
