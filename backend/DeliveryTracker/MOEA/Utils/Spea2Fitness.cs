...
namespace DeliveryTracker.MOEA.Utils
{
	...
	public class Spea2Fitness
	{
		...
		{
			this.distanceMatrix = this.distance.DistanceMatrix(solutionSet);
		...
		{
			double[] strength = new double[this.solutionSet.Size()];
			double[] rawFitness = new double[this.solutionSet.Size()];
			double kDistance;
			...
			for (int i = 0; i < this.solutionSet.Size(); i++)
			{
				...
				{
					if (this.dominance.Compare(this.solutionSet.Get(i), this.solutionSet.Get(j)) == -1)
					{
						strength[i] += 1.0;
					}
				}
			...
			for (int i = 0; i < this.distanceMatrix.Length; i++)
			{
				...
				Array.Sort(this.distanceMatrix[i]);
				kDistance = 1.0 / (this.distanceMatrix[i][k] + 2.0); // Calcule de D(i) distance
				this.solutionSet.Get(i).Fitness = rawFitness[i] + kDistance;
			}
		}
		...
		public SolutionSet EnvironmentalSelection(int size)
		{
			...
			if (this.solutionSet.Size() < size)
			{
				size = this.solutionSet.Size();
			}

			// Create a new auxiliar population for no alter the original population
			SolutionSet aux = new SolutionSet(this.solutionSet.Size());

			int i = 0;
			while (i < this.solutionSet.Size())
			{
				if (this.solutionSet.Get(i).Fitness < 1.0)
				{
					aux.Add(this.solutionSet.Get(i));
					this.solutionSet.Remove(i);
				}
				else
				{
					i++;
			...
			if (aux.Size() < size)
			{
				...
				this.solutionSet.Sort(comparator);
			...
			double[][] distanceMatrix = this.distance.DistanceMatrix(aux);
			...
			{
				aDistanceList.Sort(this.distanceNodeComparator);
